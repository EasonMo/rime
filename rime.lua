-- TRACE1=true  -- enable trace1
-- TRACE2=true  -- eanble trace2
-- GD= require 'croissant.debugger'
-- GD()  ---    break point

-- if  TRACE1 then GD() end
-- if  TRACE2 then GD() end
...
--- 过滤器：只显示单字
local break_list = {'coco', 'date', 'datetime', 'time', 'week', 'month'}
...
function single_char_only(input)
    for cand in input:iter() do
        -- log.error('cond.type:'..cand.type)
        -- log.error('cond.text:'..cand.text)
        if (has_value(break_list, cand.type) or utf8.len(cand.text) == 1) then
        -- if (utf8.len(cand.text) == 1) then
            yield(cand)
        end
    end
